...
bot.on('message', async message => {
    ...
    if (!isNaN(messages)) {
        ...
        let levelembed = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(`**LEVELED-UP** Congratulations _${message.author},_ Anda telah Naik Level ke Level ${levelfetch}`)
            .setFooter(`FarrellID | Developer Indonesia`)
...
bot.on('message', async message => {
    ...
    try {
        if (message.mentions.has(bot.user) && !message.mentions.has(message.guild.id)) {
            return message.channel.send(`**PREFIX-BOT 🖥️**- _\`${prefix}\`_`)
        }
    } catch {
        return;
    };
...
const listener = app.listen(process.env.PORT, () => {
  console.log("[☑️] Successfully Connected on Port: " + listener.address().port);
});
